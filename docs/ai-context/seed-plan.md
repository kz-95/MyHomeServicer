...
> questions. **9 servicers seeded (M97-M105)** - 3 per new category (Painting, Moving, Gardening). Non-essential questions are `required: false` (skippable).
...
For every servicer in every category, creates `ServicerModule` records matching the priced questions from that category's `questionSchema`. These modules are referenced by auto-accept advanced listings - when a customer submits a quote, the auto-accept engine matches their selected options against servicer modules to compute a price automatically.
...
The `questionKey` + `optionValue` pair must **exactly match** the `static.ts` `QuoteQuestion` definition. Any mismatch means the auto-accept engine cannot find a price for the customer's selection and auto-proposal silently fails.

### Coverage

All 34 child categories have complete module coverage. Every `priced: true` question option has a corresponding `ServicerModule` record with a default price and duration. The module definitions are in `CATEGORY_MODULES` (explicit per-category) and `GENERIC_AUTO` (appliance repair + basic training categories) in `seed.ts`.

### Single entry point

All module seeding is consolidated in `seed.ts`. The single `npm run seed` command creates everything in one pass:

```
clearAll() → categories → settings → admin → customers → servicers → modules → 
quotes → bookings → invoices → penalty data → chat → platform revenue → manifest
```

See `docs/superpowers/plans/2026-06-25-sp3-seeding-coverage.md` for the full per-category coverage table.
