...
| BOOKING_LOCATION_LOG | GPS audit for arrive/done | Created when servicer taps "arrived" or "mark done". `eventType` = `arrive`|`done`. `lat`, `lng`, `accuracy` from `navigator.geolocation`. `verified` = boolean (within 500m for arrive, 1km for done). `verifiedAt` timestamp. Backward-compatible: if GPS unavailable, arrive/done proceeds without log. |
