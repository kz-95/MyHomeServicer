...
test.describe('Scenario 7 - Availability Gating (No Dispatch)', () => {
  test.beforeAll(async ({ browser }) => {
    log = new StepLogger('07');
  ...
  test('7.1 - Login C_FRESH and clear servicer schedule via API', async () => {
    log.step('Login C_FRESH');
    await loginAs(pageC, 'C_FRESH', log);

    log.step('Clear servicer kumar.selvam@demo.local schedule for today');
      ...
      await pageC.evaluate(
        async ({ apiBase, wd, devUser }) => {
          await fetch(`${apiBase}/servicer/me/schedule`, {
            ...
            body: JSON.stringify({ slots: [{ weekday: wd, timeSlot: 'morning', available: false }] }),
          });
          await fetch(`${apiBase}/servicer/me/schedule`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json', 'x-dev-user': devUser },
            body: JSON.stringify({ slots: [{ weekday: wd, timeSlot: 'noon', available: false }] }),
          });
          await fetch(`${apiBase}/servicer/me/schedule`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json', 'x-dev-user': devUser },
            body: JSON.stringify({ slots: [{ weekday: wd, timeSlot: 'afternoon', available: false }] }),
          });
        },
        { apiBase: BACKEND, wd: weekday, devUser: 'kumar.selvam@demo.local' },
      );
      log.ok('Servicer schedule cleared for today');
    } catch (e: any) {
      log.warn('Clear schedule API', `error: ${e?.message ?? e}`);
    }

    await pageC.screenshot({ path: getScreenshotPath(SCENARIO_ID, 1) });
    log.screenshot('Servicer schedule cleared', null);
  });

  test('7.2 - Submit aircond quote', async () => {
    ...
    log.screenshot('Quote submitted with no schedule', null);
  });

  test('7.3 - Login servicer and verify no dispatch overlay', async () => {
    ...
    log.step('Wait and verify no dispatch overlay appeared');
    ...
    if (overlayCount === 0) {
      log.ok('No dispatch overlay visible', 'availability gating working correctly');
    } else {
      log.warn('Dispatch overlay', `found ${overlayCount} despite empty schedule`);
    }

    await pageS.screenshot({ path: getScreenshotPath(SCENARIO_ID, 4) });
    log.screenshot('No dispatch overlay (availability gating)', null);
  });
});
